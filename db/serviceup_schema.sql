...
-- Pixels: tenant media metadata. File bytes live in Supabase Storage.
create table if not exists public.pixels (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint pixels_storage_unique unique (bucket, storage_path)
);

-- One reusable organization/contact profile per tenant.
...
begin
  for t in
    ...
    where schemaname = 'public'
      and tablename in (
        'settings','app_settings','roles','permissions','users','pixels','organization_profiles',
