...
async function resolveContentTypeId(db, idOrSlug) {
  ...
  const { rows } = await db.query(`SELECT id FROM content_types WHERE slug = $1`, [
    raw,
  ]);
...
router.post("/", requireAdmin, async (req, res) => {
  try {
    ...
    const { rows } = await req.db.query(insertSql, [
      slug,
      type,
      label_singular,
      label_plural,
...
router.get("/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const resolvedId = await resolveContentTypeId(req.db, id);
    ...
    const fieldsResult = await req.db.query(
...
router.put("/:id", requireAdmin, async (req, res) => {
  try {
    ...
    const { rows } = await req.db.query(updateSql, [
      slug,
      label_singular,
      label_plural,
      description,
...
router.delete("/:id", requireAdmin, async (req, res) => {
  try {
    ...
    await req.db.query("DELETE FROM content_types WHERE id = $1", [resolvedId]);
