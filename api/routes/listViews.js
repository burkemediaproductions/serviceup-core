...
async function getListViewsForTypeAndRole(db, contentTypeId, role) {
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const role = roleParam ? String(roleParam).toUpperCase() : (req.user?.role || 'VIEWER').toUpperCase();
      ...
      const views = await getListViewsForTypeAndRole(req.db, contentTypeId, includeAll ? null : role);
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (Array.isArray(roles) && roles.length > 0) {
        ...
        const fallbackRole = req.user?.role || 'VIEWER';
