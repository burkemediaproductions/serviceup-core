...
router.get('/gizmos', async (req, res) => {
  ...
  try {
    ...
    const { rows } = await req.db.query(`SELECT * FROM gizmos ${where} ORDER BY created_at DESC`, params);
    res.json(rows);
  } catch (err) {
...
router.get('/gizmos/:id', async (req, res) => {
  const { id } = req.params;
  try {
    const { rows } = await req.db.query('SELECT * FROM gizmos WHERE id = $1 LIMIT 1', [id]);
...
router.delete('/gizmos/:id', async (req, res) => {
  const { id } = req.params;
  try {
    const { rows } = await req.db.query('DELETE FROM gizmos WHERE id = $1 RETURNING id', [id]);
    if (!rows.length) return res.status(404).json({ error: 'Gizmo not found' });
    res.json({ ok: true });
  } catch (err) {
