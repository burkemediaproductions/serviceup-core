...
router.get('/gadgets/:id', async (req, res) => {
  const { id } = req.params;
  try {
    const { rows } = await req.db.query('SELECT * FROM gadgets WHERE id = $1 LIMIT 1', [id]);
    if (!rows.length) return res.status(404).json({ error: 'Gadget not found' });
    const gadget = rows[0];
    const { rows: gizmoRows } = await req.db.query(
...
router.delete('/gadgets/:id', async (req, res) => {
  const { id } = req.params;
  try {
    await req.db.query('DELETE FROM gadget_gizmos WHERE gadget_id = $1', [id]);
    const { rows } = await req.db.query('DELETE FROM gadgets WHERE id = $1 RETURNING id', [id]);
    if (!rows.length) return res.status(404).json({ error: 'Gadget not found' });
    res.json({ ok: true });
  } catch (err) {
...
router.delete('/gadgets/:id/gizmos/:gizmoId', async (req, res) => {
  ...
  try {
    await req.db.query('DELETE FROM gadget_gizmos WHERE gadget_id = $1 AND gizmo_id = $2', [id, gizmoId]);
    res.json({ ok: true });
  } catch (err) {
