...
function authHeaders(extra = {}) {
  const headers = { ...extra };
  try {
    const configuredTenant = import.meta.env.VITE_SERVICEUP_TENANT;
    const storedTenant = JSON.parse(localStorage.getItem('serviceup.tenant') || 'null');
    const tenantKey = configuredTenant || storedTenant?.slug || storedTenant?.id;
    if (tenantKey) {
      headers['X-ServiceUp-Tenant'] = tenantKey;
    }
